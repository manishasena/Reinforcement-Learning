...
class AGENT:
    ...
    def __init__(self,epsilon, iterations, alpha, gamma):
        ...
        layers = [1000, 10,  1]  #N, D_in, H, D_out
    ...
    def Weight_Update(self,previous_state,R,alpha,gamma):
        ...
        delta = R + gamma*v_hat_S_dash - v_hat_S
        loss = (R + gamma*v_hat_S_dash - v_hat_S).pow(2).sum()
        loss.backward()
        
        mu = float(self.mu[int(previous_state-1)]/np.sum(self.mu))
        
        with torch.no_grad():
            self.weights['0'] -= mu*alpha*self.weights['0'].grad
            self.weights['1'] -= mu*alpha*self.weights['1'].grad
            self.bias['0'] -= mu*alpha*self.bias['0'].grad
            self.bias['1'] -= mu*alpha*self.bias['1'].grad
...
plt.plot(np.linspace(0,1000,1000).reshape(-1,1),ag.Final_State.reshape(-1,1))
plt.show()
