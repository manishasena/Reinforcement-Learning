...
class Car:
    ...
    def Action_Value(self,position,velocity,act):
        ...
        for i in range(len(self.layers)-1):
            x = (x.mm(self.weights[i]["W"])+self.weights[i]['b'])
            #x = x.mm(self.weights[i]["W"])
    ...
    def SGD_Weight_Update(self,previous_position,previous_velocity,prev_action,done,R,alpha):
        ...
        with torch.no_grad():
            for i in range(len(self.layers)-1):
                self.weights[i]['W'] += alpha*delta*self.weights[i]['W'].grad
                self.weights[i]['b'] += alpha*delta*self.weights[i]['b'].grad
